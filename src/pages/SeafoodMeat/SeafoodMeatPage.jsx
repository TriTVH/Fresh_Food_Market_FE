...
function SeafoodMeatPage() {
  ...
  const fetchSeafoodMeat = async () => {
    try {
      ...
      if (selectedSubcategory !== 'all') {
        const subcatConfig = seafoodMeatSubcategories.find(s => s.id === selectedSubcategory)
      ...
      if (!response.ok) throw new Error('Failed to fetch seafood & meat')
      
      const data = await response.json()
      setProducts(data.products || data)
      
    } catch (err) {
      console.error('Error fetching seafood & meat:', err)
  ...
  */
  ...
  const seafoodMeatSubcategories = [
    ...
    { id: 'poultry', name: 'Thịt Gà, Vịt & Chim', values: ['chicken', 'duck', 'poultry'] },
  ]

  // Get only seafood and meat products
  const allSeafoodMeat = mockProducts.meatSeafood || []
  ...
  const filteredProducts = useMemo(() => {
    return allSeafoodMeat.filter((product) => {
      // Search filter
      if (searchQuery) {
  ...
  }, [allSeafoodMeat, searchQuery, selectedSubcategory, selectedPriceRanges])
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 text-sm mb-6">
          ...
          <span className="text-gray-800 font-semibold">Thịt, Cá & Hải Sản</span>
        </div>

        {/* Page Header */}
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-2 sm:mb-3">
            Thịt, Cá & Hải Sản Fresh Market
          </h1>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-3 sm:p-4 mb-6 sm:mb-8">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3">
            ...
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm thịt, cá, hải sản..."
                value={searchQuery}
            ...
            <div className="relative">
              ...
              >
                <span className="text-gray-700 font-medium">Thịt, Cá & Hải Sản</span>
                  ...
                  {categories.map((category) => (
                    <button
                      ...
                      className={`w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50 transition-colors first:rounded-t-xl last:rounded-b-xl ${
                        category.id === 'seafood' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                      }`}
                    >
                      {category.name}
                    </button>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-2 sm:p-3 mb-6 sm:mb-8">
          <div className="flex gap-2 overflow-x-auto">
            {seafoodMeatSubcategories.map((subcat) => (
              <button
                key={subcat.id}
                onClick={() => {
...
export default SeafoodMeatPage
