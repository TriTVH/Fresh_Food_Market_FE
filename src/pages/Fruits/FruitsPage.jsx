...
function FruitsPage() {
  ...
  const filteredProducts = useMemo(() => {
    return allFruits.filter((product) => {
      // Search filter
      if (searchQuery) {
  ...
  }, [allFruits, searchQuery, selectedSubcategory, selectedPriceRanges])
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-3 sm:p-4 mb-6 sm:mb-8">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3">
            ...
            <div className="relative">
              ...
              >
                <span className="text-gray-700 font-medium">Trái Cây</span>
                  ...
                  {categories.map((category) => (
                    <button
                      ...
                      className={`w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50 transition-colors first:rounded-t-xl last:rounded-b-xl ${
                        category.id === 'fruits' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                      }`}
                    >
                      {category.name}
                    </button>
